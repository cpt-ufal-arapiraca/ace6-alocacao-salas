...
export class AutenticacaoService {
  ...
  async entrar(entrarAutenticacaoDTO: EntrarAutenticacaoDTO): Promise<any> {
    ...
    if (
      !usuario ||
      usuario.autenticacao.autenticacao_situacao !== SituacaoLoginEnum.ATIVO ||
      !(await bcrypt.compare(
        entrarAutenticacaoDTO.autenticacao_senha,
        usuario.autenticacao.autenticacao_senha,
      ))
    ) {
    ...
    const sessao = await this.prisma.sessao
      .create({
        data: {
          sessao_ip: entrarAutenticacaoDTO.sessao_ip,
          sessao_so: useragent
            .parse(entrarAutenticacaoDTO.sessao_so)
            .os.toString(),
  ...
  async alterarSenha(
    alterarSenhaAutenticacaoDTO: AlterarSenhaAutenticacaoDTO,
  ): Promise<any> {
    if (
      alterarSenhaAutenticacaoDTO.autenticacao_senha !==
    ...
    const autenticacaoBD = await this.prisma.autenticacao
      .findUnique({
        where: {
    ...
    const autenticacao = this.prisma.autenticacao
      .update({
        data: {
  ...
  async recuperarSenha(
    recuperarSenhaAutenticacaoDTO: RecuperarSenhaAutenticacaoDTO,
  ): Promise<any> {
    const usuario = await this.prisma.usuario
      .findUnique({
        where: {
  ...
  async redefinirSenha(
    redefinirSenhaAutenticacaoDTO: RedefinirSenhaAutenticacaoDTO,
  ): Promise<any> {
    const autenticacao_token: string =
      redefinirSenhaAutenticacaoDTO.autenticacao_token;
    ...
    if (!autenticacaoBD) {
      throw new HttpException(`Token não existe.`, HttpStatus.NOT_FOUND);
    }

    const saltOrRounds = 10;
    const hash = await bcrypt.hash(
      redefinirSenhaAutenticacaoDTO.autenticacao_senha,
      saltOrRounds,
    );
